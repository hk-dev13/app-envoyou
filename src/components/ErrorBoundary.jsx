      ...
      return (
        <div className="min-h-screen bg-background flex items-center justify-center py-12 px-4">
          <div className="max-w-md w-full space-y-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-primary-foreground">Something went wrong</h2>
            <p className="text-muted-foreground">
              We encountered an unexpected error. Please try refreshing the page.
            </p>
            <button
              onClick={() => window.location.reload()}
              className="w-full bg-primary hover:bg-emerald-700 text-primary-foreground font-semibold py-3 px-4 rounded-lg transition-colors"
            >
              Refresh Page
            </button>
            {showDebug && (
              <details className="mt-4 text-left">
                <summary className="text-muted-foreground cursor-pointer">Error Details (Debug)</summary>
                <pre className="mt-2 text-xs text-red-400 bg-background p-4 rounded overflow-auto whitespace-pre-wrap break-all">
